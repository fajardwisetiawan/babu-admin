  ...
  $(document).ready(function () {
            ...
            $.ajax({
              ...
              success: function(response) {
                ...
                for (let i = 0; i < response.arr.length; i++) {
                  $('#content').append('<tr> <td>'+response.arr[i].negara+'</td> <td>'+response.arr[i].code[1]+'</td> <td>'+response.arr[i].code[2]+'</td> <td><button type="button" class="btn waves-effect waves-light btn-success edit" data-toggle="modal" data-target="#exampleModal" param="'+response.arr[i].negara+'" id="'+response.arr[i]._id+'"><i class="ti-pencil"></i></button> <button type="button" class="btn waves-effect waves-light btn-danger hapus" id="'+response.arr[i]._id+'"><i class="ti-trash"></i></button></td> </tr>')
                }
              },
              error: function(e) {
                console.log(e);
      ...
      $(document).on('click','.hapus',function(){
        ...
        .then((willDelete) => {
          if (willDelete) {
            $.ajax({
              url: '<?= $link_api ?>negara/'+id+'/'+localStorage.getItem('id'),
              type: "DELETE",
              dataType: "JSON",
              success: function(response) {
      ...
      function getContent(){
        ...
        var url;
        if (page != null) {
          url = '<?= $link_api ?>negara?page='+page
        } else {
          url = '<?= $link_api ?>negara?page=1'
          page = 1
        }
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            if (totalPages < 3) {
              // loop biasa
              if (page == 2) {
                $('#pagination').append('<li class="page-item"> <a class="page-link" href="<?= $actual_link ?>pages/negara?page=1">Previous</a> </li>')
              } else {
              ...
              for (let i_pages = 1; i_pages < totalPages+1; i_pages++) {
                if (i_pages == page) {
                  $('#pagination').append('<li class="page-item active"> <a class="page-link" href="<?= $actual_link ?>pages/negara?page='+i_pages+'">'+i_pages.toLocaleString().replace(',','.')+'</a> </li>');
                } else {
                  $('#pagination').append('<li class="page-item"> <a class="page-link" href="<?= $actual_link ?>pages/negara?page='+i_pages+'">'+i_pages.toLocaleString().replace(',','.')+'</a> </li>');
                }
              }
              ...
              if (page == 2) {
                ...
                $('#pagination').append('<li class="page-item"> <a class="page-link" href="<?= $actual_link ?>pages/negara?page='+Number(parseInt(page)+1)+'">Next</a> </li>')
              }
              ...
              console.log('total page : '+totalPages);
                ...
                // alert(page3 )
              if (page2 >= totalPages) {
                $('#pagination').append('<li class="page-item"> <a class="page-link" href="<?= $actual_link ?>pages/negara?page='+page1+'">Previous</a> </li>')
              ...
              if (page2 >= totalPages) {
                ...
                $('#pagination').append('<li class="page-item disabled"> <a class="page-link" href="<?= $actual_link ?>pages/negara?page='+Number(parseInt(page)+1)+'">Next</a> </li>')
                page1 = 1
                page3 = Number(parseInt(page)+3)
              } else {
                $('#pagination').append('<li class="page-item"> <a class="page-link" href="<?= $actual_link ?>pages/negara?page='+Number(parseInt(page)+1)+'">Next</a> </li>')
          ...
          <form id="form_submit">
            ...
            <div class="modal-body">
              ...
              <div class="form-group">
                <label for="kata">Nama Negara</label>
      ...
      <div class="page-breadcrumb">
        <div class="row">
          <div class="col-7 align-self-center">
            <h3 class="page-title text-truncate text-dark font-weight-medium mb-1">Negara</h3>
            <div class="d-flex align-items-center">
              <nav aria-label="breadcrumb">
                <ol class="breadcrumb m-0 p-0">
                  <li class="breadcrumb-item"><a href="#">Manajemen Negara</a>
                  </li>
                </ol>
              </nav>
            </div>
          ...
          <div class="col-5 align-self-center">
            <div class="customize-input float-right">
              <button type="button" class="btn waves-effect waves-light btn-rounded btn-primary" data-toggle="modal" data-target="#modaltambah"><i data-feather="plus" class="feather-icon"></i> Tambah Negara</button>
            </div>
          </div>
        </div>
      </div>
